...
func atomicReplaceRelativeReaderPlatform(ctx context.Context, root, relative, target string, source io.Reader, limit int64, validate func(int64) error, hooks Hooks) error {
	...
	for attempt := 0; attempt < 100; attempt++ {
		candidate := ".sticker-stream-" + time.Now().Format("20060102150405.000000000") + "-" + string(rune('a'+attempt%26)) + ".tmp"
	...
	count, copyErr := copyContext(ctx, temporary, reader)
	if copyErr != nil {
		...
		return wrapError("io", "read_failed", "Check the source stream.", copyErr)
	}
	if validate != nil {
