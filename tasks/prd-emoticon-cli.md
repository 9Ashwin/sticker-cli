...
状态：草案，待用户评审。本文定义计划中的能力，不表示命令已经实现或发布。
...
[Assumption] 命令名采用 `emoticon`。新增 CLI 仓库公开，与现有公开素材仓库保持一致。素材仓库最终改名前集中更新 README、AGENTS、素材源及相关链接，不把永久依赖 GitHub 重定向当作迁移方案。
...
1. 命名建议：CLI `emoticon-cli` / 命令 `emoticon`，素材 `agent-emoticon-packs`；素材远端改名在名称确认后执行。
...
请评审本草案并提出调整，或回复 OK 确认。确认后可用 `/prd-to-spec` 固化命令和存储契约，再用 `/to-issues` 拆解实施任务。
