...
状态：技术设计草案，待评审；命令与结构均为实施合同，不是现有实现。
...
依赖与具体 Issue 草案见 [实施任务草案](issues-emoticon-cli.md)。该文件使用本地编号 I01…，不是 GitHub Issue 编号。
...
评审重点：标准个人库与包状态的边界、整体提交的导入规则，以及首版卸载保留原图的行为。
